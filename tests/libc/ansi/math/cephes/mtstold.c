...
/* Absolute error criterion starts with tstyp = GAMMA: */
    {"gamma", gamma, "lgamma", lgamma, 1, GAMMA, 0, 11.0, 1.0, 0,
     0.0, 0.0, 0},
    {"  acos", acos, "   cos", cos, 1, 0, 0, 2.0, -1.0, 0,
     0.0, 0.0, 0},
/* Smaller number of trials starts with tstyp = WRONK1: */
    {"  Jnt", jn, "  Ynt", yn, 2, WRONK1, 0, 30.0, 0.1, 0,
     /* 40.0, -20.0,*/ 20.0, 0.0, INT},
...
#endif
    defs[1].arg1w = CPI;
    defs[1].arg1l = -CPI/2.0;
    defs[3].arg1w = MAXLOG*FACT;
    defs[3].arg1l = -MAXLOG/2.0*FACT;
    defs[4].arg1w = 2.0*MAXLOG*FACT;
    defs[4].arg1l = -MAXLOG*FACT;
    defs[6].arg1w = 2.0*MAXLOG*FACT;
    defs[6].arg1l = -MAXLOG*FACT;
    defs[7].arg1w = MAXLOG*FACT;
    defs[7].arg1l = 0.0;
 ...
 */
	...
	printf("random arguments in %g %g\n",
...
/* peak detect the error */
		...
		if (e > 1.0e-10)
		{
		    /* printf("x %.6E z %.6E y %.6E max %.4E\n",
			   x, z, y, max); */
		    printf("x %.18E z %.18E y %.18E max %.4E\n",
			   x, z, y, max);
		    if (d->tstyp >= WRONK1)
		    {
			printf("cy1 %.4E y2 %.4E y3 %.4E y4 %.4E k %d x %.4E\n",
			       cy1, y2, y3, y4, k, x);
		    }
		}
