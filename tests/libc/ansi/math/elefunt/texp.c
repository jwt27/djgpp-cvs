...
texp()
{
	...
	for (i = 1; i <= n; ++i)
	{
	    ...
	    zz = EXP(y);
	    if (j == 1)
		z = z - z * 6.058693718652421388e-2L;
	    else
	    {
		if (ibeta != 10)
	...
	printf("\fTEST OF EXP(X-" F7P4F ") VS EXP(X)/EXP(" F7P4F ")\n\n\n", v, v);
	printf("%7d RANDOM ARGUMENTS WERE TESTED FROM THE INTERVAL\n", n);
	printf("      (" F15P4E "," F15P4E ")\n\n\n", a, b);
	printf(" EXP(X-V) WAS LARGER%6d TIMES,\n", k1);
...
" THERE ARE %4d BASE %4d SIGNIFICANT DIGITS IN A FLOATING-POINT NUMBER\n\n\n",
    ...
    x = ZERO;
    y = EXP(x) - ONE;
    printf(" EXP(0.0) - 1.0E0 = " F15P7E "\n\n", y);
    x = AINT(ALOG(xmin));
    y = EXP(x);
    printf(" EXP(" F13P6E ") =" F13P6E "\n\n", x, y);
    x = AINT(ALOG(xmax));
    y = EXP(x);
    printf(" EXP(" F13P6E ") =" F13P6E "\n\n", x, y);
    x = x / TWO;
    ...
    z = EXP(v);
    z = z * z;
    printf(" IF EXP(" F13P6E ") = " F13P6E " IS NOT ABOUT\n", x, y);
    printf(" EXP(" F13P6E ")**2 =" F13P6E " THERE IS AN ARG RED ERROR\n", v, z);

    /* test of error returns */

    printf("\fTEST OF ERROR RETURNS\n\n\n");
    x = -ONE / sqrt(xmin);
    printf(" EXP WILL BE CALLED WITH THE ARGUMENT" F15P4E "\n",x);
    ...
    if (errno)
	perror("exp()");
    printf(" EXP RETURNED THE VALUE" F13P6E "\n\n\n\n", y);
    x = -x;
    printf(" EXP WILL BE CALLED WITH THE ARGUMENT" F13P6E "\n", x);
    printf(" THIS SHOULD TRIGGER AN ERROR MESSAGE\n\n\n");
    fflush(stdout);
    errno = 0;
    y = EXP(x);
